...
client.on("messageCreate", async (message) => {
  ...
  if (command) {
    ...
    if (
      ...
      console.log(`User: ${message.author.tag} used command: ${cmd}`);
      ...
      fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
        ...
        body: JSON.stringify({
          ...
          text: `User: ${message.author.tag} used command: ${cmd} in ${message.guild.name}`,
        }),
      }).catch(console.error);
  
      // Send log to Discord logs channel
      const logChannel = message.guild.channels.cache.get(LOG_CHANNEL_ID);
      if (logChannel) {
        logChannel.send(`User: ${message.author.tag} used command: ${cmd}`);
      }
    }
  }
});
